...
_dispatch_add_pump(cJSON * const obj, char const * const key, poolstate_t const * const state)
{
    ...
    add_pump_running(item, "running", pump->running.value);
    _addPumpStateToObject(item, "state", pump->state.value);
    cJSON_AddNumberToObject(item, "power", pump->power.value);
    cJSON_AddNumberToObject(item, "speed", pump->speed.value);
    if (pump->flow.value) {
        cJSON_AddNumberToObject(item, "flow", pump->flow.value);
    }
    if (pump->level.value) {
        cJSON_AddNumberToObject(item, "level", pump->level.value);
    }
    cJSON_AddNumberToObject(item, "error", pump->error.value);
    cJSON_AddNumberToObject(item, "timer", pump->timer.value);
}
